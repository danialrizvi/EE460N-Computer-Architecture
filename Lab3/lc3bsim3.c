...
int main(int argc, char *argv[]) {
	...
	*/
	printf("Enter input file name (ucode3.txt): ");
	scanf_s("%s", ucode3, 50);
	printf("Enter input file name (.obj file): ");
	scanf_s("%s", isaprogram, 50);

	printf("LC-3b Simulator\n\n");

	initialize(ucode3, isaprogram, 1);
	/*WIN*/
...
void eval_bus_drivers() {
	...
	*/
	...
	/*InGateALU*/
	A = sext(SR1val, 0x0080);
	if (SR2MUX == 0) {
		SR2val = CURRENT_LATCHES.REGS[SR2];
		B = sext(SR2val, 0x0080);
	}
	else {
		B = imm5;
	}
...
void drive_bus() {
	...
	*/
	...
	if ((GateMARMUX == FALSE) && (GatePC == FALSE) && (GateSHF == FALSE) && (GateALU == FALSE) && (GateMDR == FALSE)) {
		BUS = 0;
	}
