...
/***************************************************************/
int GetRSS(int *x) { return(x[RSS]); }
int GetIRD(int *x) { return(x[IRD]); }
int GetMAE(int *x) { return(x[MAE]); }
...
int GetLD_RSR(int *x) { return(x[LD_RSR]); }
int GetLD_VA(int *x) { return(x[LD_VA]); }
int GetGATE_VA(int *x) { return(x[GATE_VA]); }
int GetGATE_PTBR(int *x) { return(x[GATE_PTBR]); }
int GetP2(int *x) { return(x[P2]); }
int GetMARB(int *x) { return((x[MARB1] << 1) + x[MARB0]); }
int GetMDRW(int *x) { return(x[MDRW]); }
...
char ucode4[50] = "ucode5.txt";
char pagetable[50] = "pagetable.obj";
char add[50] = "add.obj";
...
char excp4[50] = "except_pagef.obj";
...
void initialize(char *ucode_filename, char *pagetable_filename, char *program_filename, int num_prog_files) {
	...
	load_program(data, 1);
	load_program(int1, 1);
	load_program(vectable, 1);
	load_program(excp1, 1);
	load_program(excp2, 1);
	load_program(excp3, 1);
	load_program(excp4, 1);
	*/
...
int main(int argc, char *argv[]) {
	...
	initialize(ucode4, pagetable, add, 1);
	*/
...
void eval_micro_sequencer() {
	...
	int nextstate;
	int ES;
	int RSS = GetRSS(CURRENT_LATCHES.MICROINSTRUCTION);
	int MAE = GetMAE(CURRENT_LATCHES.MICROINSTRUCTION);
	int RS = (CURRENT_LATCHES.RSR & 0x3F);
	int TRAPB = ((CURRENT_LATCHES.RSR & 0x40) >> 6);
	...
	if (ECOND == 1) {
		ES = (CURRENT_LATCHES.EC3 | (CURRENT_LATCHES.EC0 & (TRAPB ^ 0x01)));
	}
	if (ECOND == 2) {
		ES = CURRENT_LATCHES.EC1;
	}
	if (ECOND == 3) {
	...
	*/
	...
	if (CURRENT_LATCHES.STATE_NUMBER == 48) {
		CURRENT_LATCHES.MDR = (CURRENT_LATCHES.PSR | 0x8000);
	}
	if (CURRENT_LATCHES.STATE_NUMBER == 41) {
		CURRENT_LATCHES.MDR = (CURRENT_LATCHES.PC);
	}
...
void eval_bus_drivers() {
	...
	*/
	...
	InGateVA = Low16Bits(CURRENT_LATCHES.VA);

	/*InGatePC*/

	InGatePTBR = Low16Bits(CURRENT_LATCHES.PTBR);
...
void drive_bus() {
	...
	*/
	...
	if ((GateMARMUX == FALSE) && (GatePC == FALSE) && (GateSHF == FALSE) && (GateALU == FALSE) && (GateMDR == FALSE) && (GatePSR == FALSE) && (GateVTR == FALSE) && (GateVA == FALSE) && (GatePTBR == FALSE)) {
		BUS = 0;
	}
...
void latch_datapath_values() {
	...
	*/
	...
	int LDRSR = GetLD_RSR(CURRENT_LATCHES.MICROINSTRUCTION);
	...
	NEXT_LATCHES.PTBR = CURRENT_LATCHES.PTBR;
	...
	if (LDRSR == FALSE) {
		NEXT_LATCHES.RSR = CURRENT_LATCHES.RSR;
	}

	int MARB = GetMARB(CURRENT_LATCHES.MICROINSTRUCTION);
	int PTBR = CURRENT_LATCHES.PTBR;
	int VA = CURRENT_LATCHES.VA;
	int MDR = CURRENT_LATCHES.MDR;
	/*LDMAR*/
	...
	if (LDMAR == TRUE) {
		...
		if (MARB == 1) {
			NEXT_LATCHES.MAR = LSR(CURRENT_LATCHES.MAR, 9);
			NEXT_LATCHES.MAR = NEXT_LATCHES.MAR << 1;
			NEXT_LATCHES.MAR = NEXT_LATCHES.MAR | (PTBR & 0xFF00);
		}
		if (MARB == 2) {
			NEXT_LATCHES.MAR = (VA & 0x01FF);
			NEXT_LATCHES.MAR = NEXT_LATCHES.MAR | (MDR & 0xFE00);
		}
	}
	...
	int MDRW = GetMDRW(CURRENT_LATCHES.MICROINSTRUCTION);
	...
	if (LDMDR == TRUE) {
		if (MIOEN == 0) {
			if (MDRW == 0) {
				...
				NEXT_LATCHES.MDR = (BUS | 0x01);
				if ((CURRENT_LATCHES.RSR & 0x80) == 0x80) {
					NEXT_LATCHES.MDR = (BUS | 0x03);
				}
			}
		}
	}
	...
	/*LDEC*/
	int IRop = ((CURRENT_LATCHES.IR & 0xF000) >> 12);
	int BUS0 = (BUS & 0x0001);
	...
	int P2 = GetP2(CURRENT_LATCHES.MICROINSTRUCTION);
	...
	if (LDEV == 1) {
		...
		if (ECOND == 1) {
			...
			if ((CURRENT_LATCHES.EC0 == 0) && (CURRENT_LATCHES.EC3 == 1)) {
				NEXT_LATCHES.EXCV = 0x02;
			}
		}
		if (ECOND == 2) {
			if (CURRENT_LATCHES.EC1 == 1) {
				NEXT_LATCHES.EXCV = 0x03;
			}
		}
		if (ECOND == 3) {
	...
	if (LDRSR == 1) {
		NEXT_LATCHES.RSR = (J & 0x003F);
		if (CURRENT_LATCHES.STATE_NUMBER == 15) {
			NEXT_LATCHES.RSR = NEXT_LATCHES.RSR | 0x40;
		}
		if ((CURRENT_LATCHES.STATE_NUMBER == 53) || (CURRENT_LATCHES.STATE_NUMBER == 54) || (CURRENT_LATCHES.STATE_NUMBER == 56) || (CURRENT_LATCHES.STATE_NUMBER == 57)) {
			NEXT_LATCHES.RSR = NEXT_LATCHES.RSR | 0x80;
		}
